...
class Client(Callbacks, SocketHandler):
    ...
    #TEST BY REAPER DELETE IF NOT WORK
    ...
    def get_online_users(self, start: int = 0, size: int = 25):
        ...
        **Returns**
            ...
            - **Fail** : :meth:`Exceptions <aminofix.lib.util.exceptions>`
        """
        response = self.session.get(f"{self.api}/g/s/user-profile?type=online&start={start}&size={size}", headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: 
            return exceptions.CheckException(response.text)
        else:
            return objects.UserProfileCountList(json.loads(response.text)).UserProfileCountList
